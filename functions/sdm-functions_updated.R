...
SDMForecast <- function(data, padding = 0.1) {
  ...
  # Load forecast data
  forecast.data <- getData(name = 'CMIP5', var = 'bio', res = 10,
                          rcp = 45, model = 'IP', year = 50,
                          path = "data/")
  #forecast.files <- list.files(path = "data/cmip5/2_5m",
                               #pattern = "*.gri$",
                               #full.names = TRUE)
  #forecast.files <- crop(x = forecast.files, y = geographic.extent)
...
SDMBioclim <- function(data, padding = 0.1) {
  ...
  bioclim.data <- getData(name = "worldclim",
                          var = "bio",
                          res = 10, # Could try for better resolution, 0.5, but would then need to provide lat & long...
                          path = "data/")
  bioclim.data <- crop(x = bioclim.data, y = geographic.extent)
  ...
  bil.files <- list.files(path = "data/wc10", 
                          pattern = "*.bil$", 
                          full.names = TRUE)
